...
days_to_delete = 30  # Number of days since the last modification to determine old containers
...
# Function to delete containers not modified within the specified number of days
def clean_old_containers(storage_account_name, storage_account_url, delete_map, status_map):
    try:
        ...
        now = datetime.utcnow()
        cutoff_date = now - timedelta(days=days_to_delete)

        # List and delete containers older than the specified number of days based on last modified time
